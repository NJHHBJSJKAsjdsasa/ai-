...
class CultivationSystem:
    ...
    def practice(self, times: int = 1) -> Dict[str, Any]:
        ...
        times = max(1, min(times, max_times))  # 确保至少修炼1次，不超过上限
        ...
        for i in range(times):
            # 检查灵力是否足够
            if self.player.stats.spiritual_power < 5:
                messages.append(f"第{i+1}次修炼：灵力不足，停止修炼")
                break
            
            # 计算获得经验（使用整数运算避免浮点误差）
            # 基础经验 * 修炼速度 * (0.9到1.1的随机波动)
            random_factor = 90 + random.randint(0, 20)  # 90-110
            exp_gain = (base_exp * speed * random_factor) // 100
            exp_gain = max(1, exp_gain)  # 确保至少获得1点经验
        ...
        # 恢复灵力（每次修炼恢复10点，但不超过上限）
        actual_times = i + 1 if 'i' in dir() else times  # 实际修炼次数
        spirit_recovery = min(actual_times * 10, self.player.stats.max_spiritual_power - self.player.stats.spiritual_power)
        self.player.stats.spiritual_power += spirit_recovery
    ...
    def attempt_breakthrough(self) -> Dict[str, Any]:
        ...
        # 确保成功率在合理范围内 [0.05, 0.95]
        success_rate = max(0.05, min(0.95, success_rate))
        
        # 消耗灵力
        if self.player.stats.spiritual_power >= 20:
            self.player.stats.spiritual_power -= 20
        else:
            # 灵力不足，大幅降低成功率
            success_rate *= 0.5
            success_rate = max(0.01, success_rate)  # 最低1%成功率
        
        # 判定结果（确保所有概率之和为1）
        death_chance = 0.05  # 5%死亡概率
        injury_chance = min(GAME_CONFIG["breakthrough"]["injury_chance"], 0.3)  # 最大30%受伤概率
        failure_chance = max(0, 1.0 - success_rate - death_chance - injury_chance)  # 剩余为普通失败
        ...
        elif roll < success_rate + death_chance:
            # 死亡
            return self._breakthrough_death()
        elif roll < success_rate + death_chance + injury_chance:
            # 受伤
